...
class NewtonPolynomial: # constructs a Newton polynomial for a given stencil S about the ith cell
    ...
    def conservativeCoefficients(self) -> None:
        ...
        for row in range(self.degree + 1):
            ...
            A_row = [fixed_quad(lambda x: (x - self.xArray[self.i]) ** column, self.xArray[idx] - 0.5 * dx, self.xArray[idx] + 0.5 * dx, n=self.degree + 1)[0] / dx for column in range(self.degree + 1)] # list comprehension form (to be tested) 
            A_list.append(A_row)
